...
The directions were "NORTH", "SOUTH", "WEST", "EAST". Clearly "NORTH" and 
...
other and can't become such. Hence the result path is 
...
def dir_reduc(arr):
  ...
  for dir in arr:
    if dir_stack and opposite_dir(dir_stack[-1], dir):
      dir_stack.pop()
    else:
